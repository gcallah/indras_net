...
for i in range(FOXES):
    env.add_creature(Fox(name="brer" + str(i), life_force=39.0, 
        repro_age=11, decay_rate=5.6, max_move=40.0, max_detect=60.0,
        rand_age=True))

for i in range(RABBITS):
    env.add_creature(Rabbit(name="bunny" + str(i), life_force=21.6, 
