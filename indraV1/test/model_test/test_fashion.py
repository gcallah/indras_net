...
class BasicTestCase(TestCase):
    ...
    def test_agent_inspect(self):
        ...
        agent = self.env.agent_inspect("hipster for tracking")
        self.assertEqual(agent.name, "hipster for tracking")

    def test_add_agent(self):
        announce('test_add_agent')
        self.env.add_agent(fm.Hipster("new added hipster", "Looking trendy",
                                      self.pa["hmax_move"],
                                      self.pa["variability"]))
        # test if the add worked!
        # test by running
        new_agent = self.env.agent_inspect("new added hipster")
